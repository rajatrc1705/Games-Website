...
def quiz_data_view(request, pk):
    ...
    return JsonResponse({"data": questions, "image": images})
...
def save_quiz_view(request, pk):
    ...
    if request.is_ajax():
        ...
        return JsonResponse({"user": str(user), "score": score})


@login_required(login_url="/login")
def result_view(request, pk):

    # fetching the result for the user who is currently logged in
    user = request.user
...
def show_quiz_result(request, pk):
    ...
    # creating the data dictionary to serve the scorecard of the user
...
def leaderdata(request):
    ...
    for i in users:
        total_score = 0
        for q in quizes:
            user_result = Result.objects.filter(user__username__startswith=i).earliest(
                "date"
            )
            try:
                quiz_wise = (
                    Result.objects.filter(user__username__startswith=i)
                    .filter(quiz__quiz_name__startswith=q)
                    .earliest("date")
                )
